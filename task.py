...
class LineTask():
	...
	def next_batch(self, batchsize):
		...
		samples_1 = []
		for angle in angles[:batchsize]:
			samples_1.append(np.array(Image.fromarray(image * 255).rotate(angle).crop(crop_dim)) / 255)
		samples_2 = []
		for angle in angles[batchsize:]:
			samples_2.append(np.array(Image.fromarray(image * 255).rotate(angle).crop(crop_dim)) / 255)
		alpha = np.random.rand(batchsize) * 0.5
		return np.array(samples_1), np.array(samples_2), alpha
